...
foreach ($vm in $vms) {                        
    ...
    $props = [ordered]@{
        ...
        "Categories"                    = $myvmdetail.metadata.categories
...
if (!(Test-Path "C:\RVTools\Nutanix-Inventory")){New-Item -ItemType Directory -Path "\\EDWPSBTOOLS01\VMExportData$\"}
#$Date = (Get-Date).tostring("yyyyMMdd")
$fullreport | Export-Csv -Path "\\EDWPSBTOOLS01\VMExportData$\nutanixdata.csv" -NoTypeInformation -UseCulture -verbose:$false 
